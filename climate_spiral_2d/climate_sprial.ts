...
(async () => {
  ...
  const [centerX, centerY] = [canvas.width * 0.5, canvas.height * 0.5];
  ...
  const renderDataPoints = () => {
    ...
    dataPointsOI.forEach(({ value, year, monthIndex }, i) => {
      ...
      const x1 =
        centerX + zeroGradRadius * (1 + currentMeanDiff) * Math.cos(angle);
      const y1 =
        centerY + zeroGradRadius * (1 + currentMeanDiff) * Math.sin(angle);

      const x2 =
        centerX + zeroGradRadius * (1 + nextMeanDiff) * Math.cos(nextAngle);
      const y2 =
        centerY + zeroGradRadius * (1 + nextMeanDiff) * Math.sin(nextAngle);
